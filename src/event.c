...
//	printf("key_press:%d\n", key_code);	
	...
	(key_code == KEY_LEFT) ? cam_turn_left(&w->env->cam, 5) : (void)w;
	(key_code == KEY_RIGHT) ? cam_turn_right(&w->env->cam, 5) : (void)w;
	(key_code == KEY_DOWN) ? cam_turn_down(&w->env->cam, 5) : (void)w;
	(key_code == KEY_UP) ? cam_turn_up(&w->env->cam, 5) : (void)w;
	return (0);
}
